...
-   Simulation study: 
    ...
    -   Simulate for all significant IVs $\beta_{X_{sim}}$ from a normal distribution $\beta_{X_{sim}} \sim N(\beta_{\hat X_{IV}}, \hat se_{X_N})$ and $\beta_{X_{sim}}$ from a normal distribution $\beta_{Y_{sim}} \sim N(\beta_{\hat Y_{IV}}, \hat se_{Y_N})$; start with $n_{sim}=100$
    -   Simulate $maf$ from a Binomial distribution with $maf_{X_{sim}} \sim Bin(maf, 2N)$ 
        - This I did not fully understand: We do not want to count the successes, but simulate the $maf$. Should we draw it from a normal distribution too?
