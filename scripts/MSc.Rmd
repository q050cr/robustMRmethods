...
- Interchanged panel A and panel B in the first version of Figure 1, used the same colours in panel B and in the legends
- Added five estimates for "BMI -> GSD": Wald estimate (FTO), Wald estimate (MC4R variant), random IVW, MRMix, weighted mode
...
```{r forestplot-MendelianRandomization-GSD, fig.width=5, fig.height=24, warning=FALSE}
p2.gsd <- mr_forest(
  mr.obj,
  alpha = 0.05,
  snp_estimates = TRUE,
...
colname_pve_U <-  colnames(est_sim)[str_detect(colnames(est_sim), pattern = "pve_U_sim")]
...
colname_IVW_theta_sim_BMI_GBC <- colnames(est_sim)[str_detect(colnames(est_sim), pattern = "IVW_theta_sim_BMI_GBC_")]
colname_IVW_theta_se_sim_BMI_GBC <- colnames(est_sim)[str_detect(colnames(est_sim), pattern = "IVW_theta_se_sim_BMI_GBC_")]
colname_MODE_theta_sim_BMI_GBC <- colnames(est_sim)[str_detect(colnames(est_sim), pattern = "MODE_theta_sim_BMI_GBC_")]
colname_MODE_theta_se_sim_BMI_GBC <- colnames(est_sim)[str_detect(colnames(est_sim), pattern = "MODE_theta_se_sim_BMI_GBC_")]
colname_CONMIX_theta_sim_BMI_GBC <- colnames(est_sim)[str_detect(colnames(est_sim), pattern = "CONMIX_theta_sim_BMI_GBC_")]
colname_CONMIX_theta_se_sim_BMI_GBC <- colnames(est_sim)[str_detect(colnames(est_sim), pattern = "CONMIX_theta_se_sim_BMI_GBC_")]
...
ggplot(plot_data1b, aes(x=n, y=value, col=name))+    # plot_data1b from section Simlating on empirical data (PVE)
  ...
  geom_boxplot(data = boxplot_overlay_dat, aes(x=sample_size_filtered, y=value_X*100, # *100 to get percent values
               ...
               alpha=0.2, size=0.2)+
  geom_boxplot(data = boxplot_overlay_dat, aes(x=sample_size_filtered, y=value_U*100, # *100 to get percent values
                                group=sample_size_filtered), 
               col=ggthemes_data$few$colors$Dark[2,2][[1]],
               alpha=0.2, size=0.2)+
...
ggplot(plot_data1b, aes(x=n, y=value, col=name))+    # plot_data1b from section Simlating on empirical data (PVE)
  ...
  geom_boxplot(data = boxplot_overlay_dat, aes(x=sample_size_filtered, y=value_U*100, # *100 to get percent values
               ...
               col=ggthemes_data$few$colors$Dark[2,2][[1]],
               alpha=0.2, size=0.2, outlier.shape = NA)+
  ggdist::stat_halfeye(data = boxplot_overlay_dat, mapping=aes(x=sample_size_filtered, y=value_U*100, group=sample_size_filtered), 
                       inherit.aes = FALSE, 
                       slab_fill=ggthemes_data$few$colors$Dark[2,2][[1]], slab_alpha = 0.2,
                       geom = "slabinterval",
                       na.rm=TRUE, trim=TRUE, adjust = .5, width = 10000, .width = 0, justification = -.3, point_colour = NA)+
  gghalves::geom_half_point(
    data = boxplot_overlay_dat, mapping=aes(x=sample_size_filtered-2000, y=value_U*100, group=sample_size_filtered), 
    inherit.aes = FALSE, size=0.2, colour=ggthemes_data$few$colors$Dark[2,2][[1]],
    side = "l", range_scale = .4, alpha = .2)+
  # outcome simulation
  geom_boxplot(data = boxplot_overlay_dat, aes(x=sample_size_filtered, y=value_Y*100, # *100 to get percent values
                                               group=sample_size_filtered), 
...
## Performance Metrics BIAS, MSE, MEAN, EMP_SE
...
## args gbc
args.gbc.ivw <- list(theta = est$estimate[est$mr_methods == "IVW"], 
                     nsim=nrow(est_sim)
                     ) 
args.gbc.mode <- list(theta = est$estimate[est$mr_methods == "mode"], 
                     nsim=nrow(est_sim)
                     )
args.gbc.conmix <- list(theta = est$estimate[est$mr_methods == "conmix"], 
                     nsim=nrow(est_sim)
                     )
## Standard Deviation
...
IVW_GBC_performance_metrics <- est_sim %>% 
  select(!!!(colname_IVW_theta_sim_BMI_GBC)) %>% 
  map_df(.f = ~ funs %>% 
           map(exec, .x, !!!args.gbc.ivw), .id = "var") %>% 
  cbind(sample_size_filtered) %>% 
  rename(N = sample_size_filtered)

MODE_GBC_performance_metrics <- est_sim %>% 
  select(!!!(colname_MODE_theta_sim_BMI_GBC)) %>% 
  map_df(.f = ~ funs %>% 
           map(exec, .x, !!!args.gbc.mode), .id = "var") %>% 
  cbind(sample_size_filtered) %>% 
  rename(N = sample_size_filtered)

CONMIX_GBC_performance_metrics <- est_sim %>% 
  select(!!!(colname_CONMIX_theta_sim_BMI_GBC)) %>% 
  map_df(.f = ~ funs %>% 
           map(exec, .x, !!!args.gbc.conmix), .id = "var") %>% 
  cbind(sample_size_filtered) %>% 
  rename(N = sample_size_filtered)
```

## Performance Metrics COVERAGE, COVERAGE-BIAS-ELIMINATED, TYPE1ERROR
...
# GBC
IVW_GBC_coverage_calc <- c()
IVW_GBC_coverage_bias_eliminated_calc <- c()
IVW_GBC_type1error_calc <- c()
MODE_GBC_coverage_calc <- c()
MODE_GBC_coverage_bias_eliminated_calc <- c()
MODE_GBC_type1error_calc <- c()
CONMIX_GBC_coverage_calc <- c()
CONMIX_GBC_coverage_bias_eliminated_calc <- c()
CONMIX_GBC_type1error_calc <- c()
for (i in 1:length(sample_size_filtered)) {
  ...
  # IVW
  col_IVW_GBC_theta <- paste0("IVW_theta_sim_BMI_GBC_", format(sample_size, scientific = FALSE))
  col_IVW_GBC_theta_se <- paste0("IVW_theta_se_sim_BMI_GBC_", format(sample_size, scientific = FALSE))
  col_IVW_GBC_mr_pval <- paste0("IVW_mr_pval_BMI_GBC_", format(sample_size, scientific = FALSE))
  # calc
  IVW_GBC_coverage_calc[i] <- coverage(thetaobs = est_sim[col_IVW_GBC_theta], thetaobs_se = est_sim[col_IVW_GBC_theta_se], theta = est$estimate[est$mr_methods == "IVW"])
  IVW_GBC_coverage_bias_eliminated_calc[i] <- coverage_bias_eliminated(thetaobs = est_sim[[col_IVW_GBC_theta]], thetaobs_se = est_sim[[col_IVW_GSD_theta_se]])
  IVW_GBC_type1error_calc[i] <- type1error(est_sim[[col_IVW_GBC_mr_pval]])
  # MODE
  col_MODE_GBC_theta <- paste0("MODE_theta_sim_BMI_GBC_", format(sample_size, scientific = FALSE))
  col_MODE_GBC_theta_se <- paste0("MODE_theta_se_sim_BMI_GBC_", format(sample_size, scientific = FALSE))
  col_MODE_GBC_mr_pval <- paste0("MODE_mr_pval_BMI_GBC_", format(sample_size, scientific = FALSE))
  # calc
  MODE_GBC_coverage_calc[i] <- coverage(thetaobs = est_sim[col_MODE_GBC_theta], 
                                       thetaobs_se = est_sim[col_MODE_GBC_theta_se], 
                                       theta = est$estimate[est$mr_methods == "mode"])
  MODE_GBC_coverage_bias_eliminated_calc[i] <- coverage_bias_eliminated(thetaobs = est_sim[[col_MODE_GBC_theta]], 
                                                                       thetaobs_se = est_sim[[col_MODE_GBC_theta_se]])
  MODE_GBC_type1error_calc[i] <- type1error(est_sim[[col_MODE_GBC_mr_pval]])
  # CONMIX
  col_CONMIX_GBC_theta <- paste0("CONMIX_theta_sim_BMI_GBC_", format(sample_size, scientific = FALSE))
  col_CONMIX_GBC_theta_se <- paste0("CONMIX_theta_se_sim_BMI_GBC_", format(sample_size, scientific = FALSE))
  col_CONMIX_GBC_mr_pval <- paste0("CONMIX_mr_pval_BMI_GBC_", format(sample_size, scientific = FALSE))
  # calc
  CONMIX_GBC_coverage_calc[i] <- coverage(thetaobs = est_sim[col_CONMIX_GBC_theta], 
                                       thetaobs_se = est_sim[col_CONMIX_GBC_theta_se], 
                                       theta = est$estimate[est$mr_methods == "conmix"])
  CONMIX_GBC_coverage_bias_eliminated_calc[i] <- coverage_bias_eliminated(thetaobs = est_sim[[col_CONMIX_GBC_theta]], 
                                                                       thetaobs_se = est_sim[[col_CONMIX_GBC_theta_se]])
  CONMIX_GBC_type1error_calc[i] <- type1error(est_sim[[col_CONMIX_GBC_mr_pval]])  
}
```

```{r add-additional-metrics-to-df}
## GSD
...
MODE_GSD_performance_metrics <- MODE_GSD_performance_metrics %>% 
  select(N, everything()) %>% 
  cbind(data.frame(MODE_GSD_coverage_calc, MODE_GSD_coverage_bias_eliminated_calc, MODE_GSD_type1error_calc)) %>% 
...
CONMIX_GSD_performance_metrics <- CONMIX_GSD_performance_metrics %>% 
  select(N, everything()) %>% 
  cbind(data.frame(CONMIX_GSD_coverage_calc, CONMIX_GSD_coverage_bias_eliminated_calc, CONMIX_GSD_type1error_calc)) %>% 
...
## GBC
IVW_GBC_performance_metrics <- IVW_GBC_performance_metrics %>% 
  select(N, everything()) %>% 
  cbind(data.frame(IVW_GBC_coverage_calc, IVW_GBC_coverage_bias_eliminated_calc, IVW_GBC_type1error_calc)) %>% 
  rename(coverage = IVW_GBC_coverage_calc, 
         coverage_bias_eliminated = IVW_GBC_coverage_bias_eliminated_calc, 
         type1error = IVW_GBC_type1error_calc) %>% 
  as_tibble()
MODE_GBC_performance_metrics <- MODE_GBC_performance_metrics %>% 
  select(N, everything()) %>% 
  cbind(data.frame(MODE_GBC_coverage_calc, MODE_GBC_coverage_bias_eliminated_calc, MODE_GBC_type1error_calc)) %>% 
  rename(coverage = MODE_GBC_coverage_calc, 
         coverage_bias_eliminated = MODE_GBC_coverage_bias_eliminated_calc, 
         type1error = MODE_GBC_type1error_calc) %>% 
  as_tibble()
CONMIX_GBC_performance_metrics <- CONMIX_GBC_performance_metrics %>% 
  select(N, everything()) %>% 
  cbind(data.frame(CONMIX_GBC_coverage_calc, CONMIX_GBC_coverage_bias_eliminated_calc, CONMIX_GBC_type1error_calc)) %>% 
  rename(coverage = CONMIX_GBC_coverage_calc, 
         coverage_bias_eliminated = CONMIX_GBC_coverage_bias_eliminated_calc, 
         type1error = CONMIX_GBC_type1error_calc) %>% 
  as_tibble()
```
...
## GBC
filename_IVW_GBC_performance_metrics <- paste0("../output/performance_metrics/", 
                                               Sys.Date(), "_IVW_GBC_performance_metrics_NSIM_", nsim, ".csv")
filename_MODE_GBC_performance_metrics <- paste0("../output/performance_metrics/", 
                                               Sys.Date(), "_MODE_GBC_performance_metrics_NSIM_", nsim, ".csv")
filename_CONMIX_GBC_performance_metrics <- paste0("../output/performance_metrics/", 
                                               Sys.Date(), "_CONMIX_GBC_performance_metrics_NSIM_", nsim, ".csv")
write.csv2(x = IVW_GBC_performance_metrics, file = filename_IVW_GBC_performance_metrics)
write.csv2(x = MODE_GBC_performance_metrics, file = filename_MODE_GBC_performance_metrics)
write.csv2(x = CONMIX_GBC_performance_metrics, file = filename_CONMIX_GBC_performance_metrics)
```
...
ggarrange(IVW_GSD_simplot_thetas,MODE_GSD_simplot_thetas,CONMIX_GSD_simplot_thetas, 
          ncol = 1, nrow = 3) -> GSD_simplot_thetas_arrange
...
```{r plot-performance-metrics-bias-mse-GSD, fig.height=10, fig.width=5}
## IVW
#BIAS
...
ggarrange(bias_mse_plot_IVW_GSD, bias_mse_plot_MODE_GSD, bias_mse_plot_CONMIX_GSD,
          ncol = 1, nrow = 3, common.legend = TRUE) -> bias_mse_plot_GSD
bias_mse_plot_GSD
...
### GBC simulated theta distribution

```{r theta-sim-distribution-GBC, fig.height=6, fig.width=10, warning=FALSE}
## IVW
IVW_BMI_GBC_geom_point_dat <- est_sim %>% 
  select(!!!(colname_IVW_theta_sim_BMI_GBC)) %>% 
  pivot_longer(cols = everything(), names_to = "theta_sim") %>% 
  mutate(theta_sim_numeric = as.numeric(stringr::str_extract(string =theta_sim, pattern = "\\d+")))

ggplot(IVW_GBC_performance_metrics, aes(x=N, y=mean))+ 
  geom_point()+
  geom_hline(yintercept=  args.gbc.ivw$theta, col=ggthemes_data$few$colors$Dark[9,2][[1]], alpha=0.6, linetype=2, size=0.8)+
  annotate(geom = "text", x = 50000, y = args.gbc.ivw$theta+0.05,
         label= expression( paste("ref: ", theta[X] ))  , 
         color= ggthemes_data$few$colors$Dark[9,2][[1]]
           )+
  geom_errorbar(aes(ymin=mean-emp_se, ymax=mean+emp_se), 
                ...
                position = "dodge")+
  ## ADD rainbow plots
  geom_boxplot(data = IVW_BMI_GBC_geom_point_dat, aes(x=theta_sim_numeric, y=value, group=theta_sim_numeric), 
               col=ggthemes_data$few$colors$Dark[3,2][[1]],
               alpha=0.2, size=0.2, outlier.shape = NA)+
  ggdist::stat_halfeye(data = IVW_BMI_GBC_geom_point_dat, mapping=aes(x=theta_sim_numeric, y=value, group=theta_sim_numeric), 
                       inherit.aes = FALSE, 
                       slab_fill=ggthemes_data$few$colors$Dark[3,2][[1]], slab_alpha = 0.2,
                       geom = "slabinterval",
                       na.rm=TRUE, trim=TRUE, adjust = .5, width = 10000, .width = 0, justification = -.3, point_colour = NA)+
  gghalves::geom_half_point(
    data = IVW_BMI_GBC_geom_point_dat, mapping=aes(x=theta_sim_numeric-2000, y=value, group=theta_sim_numeric), 
    inherit.aes = FALSE, size=0.2, colour=ggthemes_data$few$colors$Dark[3,2][[1]],
    side = "l", range_scale = .4, alpha = .2)+
  ## FINISH rainbow
  scale_x_continuous(labels = comma)+
  xlab("Size of Sample I")+
  ylab("Mean thetas")+
  labs(title = "Simulated Thetas with Empirical Standard Error", subtitle = "IVW estimate: BMI -> GBC")+
  theme_few() -> IVW_GBC_simplot_thetas
IVW_GBC_simplot_thetas

## MODE
MODE_BMI_GBC_geom_point_dat <- est_sim %>% 
  select(!!!(colname_MODE_theta_sim_BMI_GBC)) %>% 
  pivot_longer(cols = everything(), names_to = "theta_sim") %>% 
  mutate(theta_sim_numeric = as.numeric(stringr::str_extract(string =theta_sim, pattern = "\\d+")))

ggplot(MODE_GBC_performance_metrics, aes(x=N, y=mean))+ 
  geom_point()+
  geom_hline(yintercept=  args.gbc.mode$theta, col=ggthemes_data$few$colors$Dark[9,2][[1]], alpha=0.6, linetype=2, size=0.8)+
  annotate(geom = "text", x = 50000, y = args.gbc.mode$theta+0.05,
         label= expression( paste("ref: ", theta[X] ))  , 
         color= ggthemes_data$few$colors$Dark[9,2][[1]]
           )+
  geom_errorbar(aes(ymin=mean-emp_se, ymax=mean+emp_se), 
                ...
                position = "dodge")+
  ## ADD rainbow plots
  geom_boxplot(data = MODE_BMI_GBC_geom_point_dat, aes(x=theta_sim_numeric, y=value, group=theta_sim_numeric), 
               col=ggthemes_data$few$colors$Dark[3,2][[1]],
               alpha=0.2, size=0.2, outlier.shape = NA)+
  ggdist::stat_halfeye(data = MODE_BMI_GBC_geom_point_dat, mapping=aes(x=theta_sim_numeric, y=value, group=theta_sim_numeric), 
                       inherit.aes = FALSE, 
                       slab_fill=ggthemes_data$few$colors$Dark[3,2][[1]], slab_alpha = 0.2,
                       geom = "slabinterval",
                       na.rm=TRUE, trim=TRUE, adjust = .5, width = 10000, .width = 0, justification = -.3, point_colour = NA)+
  gghalves::geom_half_point(
    data = MODE_BMI_GBC_geom_point_dat, mapping=aes(x=theta_sim_numeric-2000, y=value, group=theta_sim_numeric), 
    inherit.aes = FALSE, size=0.2, colour=ggthemes_data$few$colors$Dark[3,2][[1]],
    side = "l", range_scale = .4, alpha = .2)+
  ## FINISH rainbow
  scale_x_continuous(labels = comma)+
  xlab("Size of Sample I")+
  ylab("Mean thetas")+
  labs(title = "Simulated Thetas with Empirical Standard Error", subtitle = "MODE estimate: BMI -> GBC")+
  theme_few() -> MODE_GBC_simplot_thetas
MODE_GBC_simplot_thetas

## CONMIX
CONMIX_BMI_GBC_geom_point_dat <- est_sim %>% 
  select(!!!(colname_CONMIX_theta_sim_BMI_GBC)) %>% 
  pivot_longer(cols = everything(), names_to = "theta_sim") %>% 
  mutate(theta_sim_numeric = as.numeric(stringr::str_extract(string =theta_sim, pattern = "\\d+")))

ggplot(CONMIX_GBC_performance_metrics, aes(x=N, y=mean))+ 
  geom_point()+
  geom_hline(yintercept=  args.gbc.conmix$theta, col=ggthemes_data$few$colors$Dark[9,2][[1]], alpha=0.6, linetype=2, size=0.8)+
  annotate(geom = "text", x = 50000, y = args.gbc.conmix$theta+0.05,
         label= expression( paste("ref: ", theta[X] ))  , 
         color= ggthemes_data$few$colors$Dark[9,2][[1]]
           )+
  geom_errorbar(aes(ymin=mean-emp_se, ymax=mean+emp_se), 
                ...
                position = "dodge")+
  ## ADD rainbow plots
  geom_boxplot(data = CONMIX_BMI_GBC_geom_point_dat, aes(x=theta_sim_numeric, y=value, group=theta_sim_numeric), 
               col=ggthemes_data$few$colors$Dark[3,2][[1]],
               alpha=0.2, size=0.2, outlier.shape = NA)+
  ggdist::stat_halfeye(data = CONMIX_BMI_GBC_geom_point_dat, mapping=aes(x=theta_sim_numeric, y=value, group=theta_sim_numeric), 
                       inherit.aes = FALSE, 
                       slab_fill=ggthemes_data$few$colors$Dark[3,2][[1]], slab_alpha = 0.2,
                       geom = "slabinterval",
                       na.rm=TRUE, trim=TRUE, adjust = .5, width = 10000, .width = 0, justification = -.3, point_colour = NA)+
  gghalves::geom_half_point(
    data = CONMIX_BMI_GBC_geom_point_dat, mapping=aes(x=theta_sim_numeric-2000, y=value, group=theta_sim_numeric), 
    inherit.aes = FALSE, size=0.2, colour=ggthemes_data$few$colors$Dark[3,2][[1]],
    side = "l", range_scale = .4, alpha = .2)+
  ## FINISH rainbow
  scale_x_continuous(labels = comma)+
  xlab("Size of Sample I")+
  ylab("Mean thetas")+
  labs(title = "Simulated Thetas with Empirical Standard Error", subtitle = "CONMIX estimate: BMI -> GBC")+
  theme_few() -> CONMIX_GBC_simplot_thetas
CONMIX_GBC_simplot_thetas

# SAVE
filename_IVW_GBC_simplot_thetas <- paste0("../output/plots/", Sys.Date(), "_IVW_GBC_simplot_thetas_NSIM_", nsim,".png")
ggsave(filename = filename_IVW_GBC_simplot_thetas, plot = IVW_GBC_simplot_thetas, 
       width = 15, height = 6, 
       units = "in"  # default
       )
filename_MODE_GBC_simplot_thetas <- paste0("../output/plots/", Sys.Date(), "_MODE_GBC_simplot_thetas_NSIM_", nsim,".png")
ggsave(filename = filename_MODE_GBC_simplot_thetas, plot = MODE_GBC_simplot_thetas, 
       width = 15, height = 6, 
       units = "in"  # default
       )
filename_CONMIX_GBC_simplot_thetas <- paste0("../output/plots/", Sys.Date(), "_CONMIX_GBC_simplot_thetas_NSIM_", nsim,".png")
ggsave(filename = filename_CONMIX_GBC_simplot_thetas, plot = CONMIX_GBC_simplot_thetas, 
       width = 15, height = 6, 
       units = "in"  # default
       )
```

```{r theta-sim-distribution-GBC-arrange, fig.height=18, fig.width=10, warning=FALSE}
IVW_GBC_simplot_thetas$labels$title <- ""
MODE_GBC_simplot_thetas$labels$title <- ""
CONMIX_GBC_simplot_thetas$labels$title <- ""
ggarrange(IVW_GBC_simplot_thetas,MODE_GBC_simplot_thetas,CONMIX_GBC_simplot_thetas, 
          ncol = 1, nrow = 3) -> GBC_simplot_thetas_arrange

filename_GBC_simplot_thetas_arrange <- paste0("../output/plots/", Sys.Date(), "_GBC_simplot_thetas_NSIM_", nsim,".png")
ggsave(filename = filename_GBC_simplot_thetas_arrange, plot = GBC_simplot_thetas_arrange, 
       width = 10, height = 18, 
       units = "in"  # default
       )
```
...
```{r plot-performance-metrics-bias-mse-GBC, fig.height=10, fig.width=5}
## IVW
#BIAS
ggplot(IVW_GBC_performance_metrics, aes(x=N, y=bias))+ 
  geom_point()+
  geom_hline(yintercept=  0, col=ggthemes_data$few$colors$Dark[9,2][[1]], alpha=0.6, linetype=2, size=0.8)+
  scale_x_continuous(labels = comma)+
  scale_y_continuous(limits = c(-1.5,1.5))+
  xlab("Size of Sample I")+
  ylab("Bias")+
  labs(title = "IVW estimate: BMI -> GBC")+
  theme_few() -> biasplot_IVW_GBC
#MSE
ggplot(IVW_GBC_performance_metrics, aes(x=N, y=mse))+ 
  geom_point()+
    geom_hline(yintercept=  0, col=ggthemes_data$few$colors$Dark[9,2][[1]], alpha=0.6, linetype=2, size=0.8)+
  scale_y_continuous(limits = c(0,1.8))+
  scale_x_continuous(labels = comma)+
  ...
  ylab("MSE")+
  labs(title = "IVW estimate: BMI -> GBC")+
  theme_few() -> mseplot_IVW_GBC
ggarrange(biasplot_IVW_GBC, mseplot_IVW_GBC) -> bias_mse_plot_IVW_GBC

## MODE
ggplot(MODE_GBC_performance_metrics, aes(x=N, y=bias))+ 
  geom_point()+
  geom_hline(yintercept=  0, col=ggthemes_data$few$colors$Dark[9,2][[1]], alpha=0.6, linetype=2, size=0.8)+
  scale_x_continuous(labels = comma)+
  scale_y_continuous(limits = c(-1.5,1.5))+
  xlab("Size of Sample I")+
  ylab("Bias")+
  labs(title = "MODE estimate: BMI -> GBC")+
  theme_few() -> biasplot_MODE_GBC
ggplot(MODE_GBC_performance_metrics, aes(x=N, y=mse))+ 
  geom_point()+
    geom_hline(yintercept=  0, col=ggthemes_data$few$colors$Dark[9,2][[1]], alpha=0.6, linetype=2, size=0.8)+
  scale_x_continuous(labels = comma)+
    scale_y_continuous(limits = c(0,1.8))+
  ...
  ylab("MSE")+
  labs(title = "MODE estimate: BMI -> GBC")+
  theme_few() -> mseplot_MODE_GBC
ggarrange(biasplot_MODE_GBC, mseplot_MODE_GBC) -> bias_mse_plot_MODE_GBC

## CONMIX
ggplot(CONMIX_GBC_performance_metrics, aes(x=N, y=bias))+ 
  geom_point()+
  geom_hline(yintercept=  0, col=ggthemes_data$few$colors$Dark[9,2][[1]], alpha=0.6, linetype=2, size=0.8)+
  scale_x_continuous(labels = comma)+
  scale_y_continuous(limits = c(-1.5,1.5))+
  xlab("Size of Sample I")+
  ylab("Bias")+
  labs(title = "CONMIX estimate: BMI -> GBC")+
  theme_few() -> biasplot_CONMIX_GBC
ggplot(CONMIX_GBC_performance_metrics, aes(x=N, y=mse))+ 
  geom_point()+
    geom_hline(yintercept=  0, col=ggthemes_data$few$colors$Dark[9,2][[1]], alpha=0.6, linetype=2, size=0.8)+
  scale_x_continuous(labels = comma)+
  scale_y_continuous(limits = c(0,1.8))+
  ...
  ylab("MSE")+
  labs(title = "CONMIX estimate: BMI -> GBC")+
  theme_few() -> mseplot_CONMIX_GBC
ggarrange(biasplot_CONMIX_GBC, mseplot_CONMIX_GBC) -> bias_mse_plot_CONMIX_GBC

ggarrange(bias_mse_plot_IVW_GBC, bias_mse_plot_MODE_GBC, bias_mse_plot_CONMIX_GBC,
          ncol = 1, nrow = 3, common.legend = TRUE) -> bias_mse_plot_GBC
bias_mse_plot_GBC

filename_GBC_simplot_bias_mse <- paste0("../output/plots/", Sys.Date(), "_GBC_simplot_bias_mse_NSIM_", nsim, ".png")
ggsave(filename = filename_GBC_simplot_bias_mse, plot = bias_mse_plot_GBC, 
       width = 7, height = 10, 
       units = "in"  # default
       )
```
