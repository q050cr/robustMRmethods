...
Calculating p-values from a normal distribution. The se, pval and study size depend on each other, within each study. The selection of IVs should rely on the exposure study - BMI in our case. Summary statistics on genetic associations with BMI will be based on results of the GIANT consortium + GERA (n=334487 (100,418 + 234,069) individuals and 289 associated variants with p < 10-8) **(insert citation)**. We vary the sample size and calculate the number of IVs and the explained variance. *(Justo Mail 2022-09-27)*
\
Steps:
1.       Calculate more accurates SEs using the betas and Pvals provided to you (this will be the original/initial SE for n=334.487). 
...
For the selected IVs, please also calculate the explained variance for the confounder and outcome.
...
- Let’s try to estimate parameters for the simulation (pi1, varx, pi2, varu, muy, vary, pi3, pi4, delta0) based on real data. Maybe you could start with N=10.000 and N=300.000 (I will help with this)
