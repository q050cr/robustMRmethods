...
# fig01.update   # old formula for PVE exposure X (without sample size adjusting)
# 
...
# SAVE
filenamefig01_a <- paste0("../output/plots/", Sys.Date(), "_figure01_old_PVE_X_formula.png")
...
plotfigure01 <- function(data, effect_size="strong", maf="rare"){
  ...
  ggplot(plot_data1, aes(x=n, y=value, col=name))+
    ...
    scale_x_continuous(name = paste0("Size of sample ", as.roman(1)),  # 2-sample-MR or label "Study Size"
                       ...
                       limits = c(10000, 300000))+
    #ylab("% variance explained by IVs")+
    labs(title = "", subtitle = plotsubtitle,
         col='', 
         caption = "Relationship between sample size, average number of instrumental variables (IVs) and variance of traits   explained by the IVs"
         ) +
    scale_y_continuous(
...
```{r add-additional-metrics-to-df-and-print-GBC}
## GBC --------------------------
IVW_GBC_performance_metrics <- IVW_GBC_performance_metrics %>% 
  select(N, everything()) %>% 
  cbind(data.frame(IVW_GBC_coverage_calc, 
                   IVW_GBC_coverage_bias_eliminated_calc, IVW_GBC_type1error_calc)) %>% 
  ...
  as_tibble()
IVW_GBC_performance_metrics %>% 
  mutate(across(cols = where(is.numeric), round(3))) %>% 
  kable(caption = "IVW method on BMI -> GBC performance metrics") %>% 
  kable_styling(font_size=10)

MODE_GBC_performance_metrics <- MODE_GBC_performance_metrics %>% 
  select(N, everything()) %>% 
  cbind(data.frame(MODE_GBC_coverage_calc, 
                   MODE_GBC_coverage_bias_eliminated_calc, MODE_GBC_type1error_calc)) %>% 
  ...
  as_tibble()
MODE_GBC_performance_metrics %>% 
  mutate(across(cols = where(is.numeric), round(3))) %>% 
  kable(caption = "MODE method on BMI -> GBC performance metrics") %>% 
  kable_styling(font_size=10)
CONMIX_GBC_performance_metrics <- CONMIX_GBC_performance_metrics %>% 
  select(N, everything()) %>% 
  cbind(data.frame(CONMIX_GBC_coverage_calc, 
                   CONMIX_GBC_coverage_bias_eliminated_calc, CONMIX_GBC_type1error_calc)) %>% 
  ...
  as_tibble()
CONMIX_GBC_performance_metrics %>% 
  mutate(across(cols = where(is.numeric), round(3))) %>% 
  kable(caption = "CONMIX method on BMI -> GBC performance metrics") %>% 
  kable_styling(font_size=10)
```
...
```{r plot-performance-metrics-bias-mse-GBC, fig.height=10, fig.width=8}
## IVW
#BIAS
