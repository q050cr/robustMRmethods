...
###
# to knit the doc go to 'render_param_reports_msc.R'
###
...
knitr::opts_chunk$set(echo = FALSE, 
                      fig.path = figurespath,  # still a rmd relative file structure here
                      dev = "svg")
...
plot_data1b <- df_cum_expl_var %>% 
  pivot_longer(cols = c(cum_expl_varX2, cum_expl_varU, 
                        #cum_expl_varY
                        )) %>% 
  mutate(name = factor(name, labels = c("% var(Y)", "% var(X)")) #, "% var(Y)")),   # change made on 2023-04-18
...
p1.gsd+
  xlab("SNP effect on BMI")+
  ylab("SNP effect on GSD")+
...
# # ANALYSIS SET 2023-04-20 NSIM = 100
# est_sim <- readRDS(file = "./output/Rdata/sim-results/2023-04-20-nsim100/2023-04-20_est_sim_purrr_NSIM_100.rds")
# est_sim_architectures <- readRDS(file = "./output/Rdata/sim-results/2023-04-20-nsim100/2023-04-20_est_sim_architectures_purrr_NSIM_100.rds")

# ANALYSIS SET 2023-04-27 NSIM = 500
est_sim <- readRDS(file = "./output/Rdata/sim-results/2023-04-27-nsim500/2023-04-24_est_sim_purrr_NSIM_500.rds")
est_sim_architectures <- readRDS(file = "./output/Rdata/sim-results/2023-04-27-nsim500/2023-04-27_est_sim_architectures_purrr_NSIM_500.rds")


nsim <- nrow(est_sim)
nsim_architectures <- nrow(est_sim_architectures[[1]])

# all(is.na(est_sim$PRESSO_theta_se_sim_BMI_GSD_200000))
# all(is.na(est_sim_architectures[[1]]$PRESSO_mr_pval_BMI_GSD_200000))
# est_sim_PRESSO <- readRDS("./output/Rdata/_old/2022-12-23_est_sim_NSIM_100.rds")
...
```{r parallel-performance, warning=FALSE}

arrangedplot.thetas.all <- list()
...
for (true_method in 1:length(selected_true_methods)) {
  ...
  if (selected_true_methods[true_method] == "MRMix") {
    color.dash <- "black"
  } else if (selected_true_methods[true_method] == "FTO_Wald"){
    color.dash <- "blue"
  } else {
    color.dash <- ggthemes_data$few$colors$Dark[9,2][[1]]
  }
  base_size_theta_plot <- 6
  ...
  ggplot(IVW_GSD_performance_metrics, aes(x=N, y=IVW_GSD_mean))+ 
    geom_point()+
    geom_hline(yintercept=  args.gsd.method$theta, col=color.dash, alpha=0.6, linetype=2, size=0.8)+
    ...
    theme_few(base_size_theta_plot) -> IVW_GSD_simplot_thetas
  ...
  ggplot(MODE_GSD_performance_metrics, aes(x=N, y=MODE_GSD_mean))+ 
    ...
    theme_few(base_size_theta_plot) -> MODE_GSD_simplot_thetas
  ...
  ggplot(CONMIX_GSD_performance_metrics, aes(x=N, y=CONMIX_GSD_mean))+ 
    ...
    theme_few(base_size_theta_plot) -> CONMIX_GSD_simplot_thetas
  ...
  ggplot(MRMIX_GSD_performance_metrics, aes(x=N, y=MRMIX_GSD_mean))+ 
    ...
    theme_few(base_size_theta_plot) -> MRMIX_GSD_simplot_thetas
  ...
  ggplot(MEDIAN_GSD_performance_metrics, aes(x=N, y=MEDIAN_GSD_mean))+ 
    ...
    theme_few(base_size_theta_plot) -> MEDIAN_GSD_simplot_thetas
  ...
  ggplot(EGGER_GSD_performance_metrics, aes(x=N, y=EGGER_GSD_mean))+ 
    ...
    theme_few(base_size_theta_plot) -> EGGER_GSD_simplot_thetas
  ## 7 ROBUST
  ...
  ggplot(ROBUST_GSD_performance_metrics, aes(x=N, y=ROBUST_GSD_mean))+ 
    ...
    theme_few(base_size_theta_plot) -> ROBUST_GSD_simplot_thetas
  ## 8 PRESSO
  ...
  ggplot(PRESSO_GSD_performance_metrics, aes(x=N, y=PRESSO_GSD_mean))+ 
    ...
    theme_few(base_size_theta_plot) -> PRESSO_GSD_simplot_thetas
  ## 9 RAPS
  ...
  ggplot(RAPS_GSD_performance_metrics, aes(x=N, y=RAPS_GSD_mean))+ 
    ...
    theme_few(base_size_theta_plot) -> RAPS_GSD_simplot_thetas
  ## 10 LASSO
  ...
  ggplot(LASSO_GSD_performance_metrics, aes(x=N, y=LASSO_GSD_mean))+ 
    ...
    theme_few(base_size_theta_plot) -> LASSO_GSD_simplot_thetas
  ...
  arrangedplot.thetas.all[[true_method]] <- GSD_simplot_thetas_arrange
...
names(arrangedplot.thetas.all) <- selected_true_methods

arrangedthetaplot.fto.mrmix <- ggarrange(arrangedplot.thetas.all$FTO_Wald, arrangedplot.thetas.all$MRMix, ncol=2,
          labels = c("A", "B"), legend = "none", align = "hv")

if (SAVE.files ==TRUE ){
    filename_arrangedthetaplot.fto.mrmix <- paste0("./output/plots/theta_sims/", Sys.Date(), "_GSD_arranged_thetas_REF_METHOD_FTO_MRMIX_NSIM_",nsim, ".svg")
    ggsave(filename = filename_arrangedthetaplot.fto.mrmix, plot = arrangedthetaplot.fto.mrmix, 
           width = 7, height = 24, 
           units = "in"  # default
           )
  }
...
```{r parallel-performance-genetic-architectures, warning=FALSE}
effect_size <- c("strong", "weak")
maf <- c("rare", "common")
all_quadrants <- tidyr::crossing(effect_size, maf)
...
for (my_architecture in 1:length(est_sim_architectures)) {
  ...
  for (true_method in 1:length(selected_true_methods)) {
    ...
    ggplot(IVW_GSD_performance_metrics, aes(x=N, y=IVW_GSD_mean))+ 
      ...
      theme_few(base_size = base_size_theta_plot) -> IVW_GSD_simplot_thetas
    ...
    ggplot(MODE_GSD_performance_metrics, aes(x=N, y=MODE_GSD_mean))+ 
      ...
      labs(title = "Simulated Thetas with Empirical Standard Error", 
           subtitle = paste0("MODE estimate: BMI -> GSD\nSelected Reference Method: ", 
                             selected_true_methods[true_method], 
                             " estimate calculated with total sample size\nGenetic Architecture: ", name_GenArch))+
      theme_few(base_size = base_size_theta_plot) -> MODE_GSD_simplot_thetas
    ...
    ggplot(CONMIX_GSD_performance_metrics, aes(x=N, y=CONMIX_GSD_mean))+ 
      ...
      labs(title = "Simulated Thetas with Empirical Standard Error", 
           subtitle = paste0("CONMIX estimate: BMI -> GSD\nSelected Reference Method: ", 
                             selected_true_methods[true_method], 
                             " estimate calculated with total sample size\nGenetic Architecture: ", name_GenArch))+
      theme_few(base_size = base_size_theta_plot) -> CONMIX_GSD_simplot_thetas
    ...
    ggplot(MRMIX_GSD_performance_metrics, aes(x=N, y=MRMIX_GSD_mean))+ 
      ...
      labs(title = "Simulated Thetas with Empirical Standard Error", 
           subtitle = paste0("MRMIX estimate: BMI -> GSD\nSelected Reference Method: ", 
                             selected_true_methods[true_method], 
                             " estimate calculated with total sample size\nGenetic Architecture: ", name_GenArch))+
      theme_few(base_size = base_size_theta_plot) -> MRMIX_GSD_simplot_thetas
    ...
    ggplot(MEDIAN_GSD_performance_metrics, aes(x=N, y=MEDIAN_GSD_mean))+ 
      ...
      labs(title = "Simulated Thetas with Empirical Standard Error", 
           subtitle = paste0("MEDIAN estimate: BMI -> GSD\nSelected Reference Method: ", 
                             selected_true_methods[true_method], 
                             " estimate calculated with total sample size\nGenetic Architecture: ", name_GenArch))+
      theme_few(base_size = base_size_theta_plot) -> MEDIAN_GSD_simplot_thetas
    ...
    ggplot(EGGER_GSD_performance_metrics, aes(x=N, y=EGGER_GSD_mean))+ 
      ...
      labs(title = "Simulated Thetas with Empirical Standard Error", 
           subtitle = paste0("EGGER estimate: BMI -> GSD\nSelected Reference Method: ", 
                             selected_true_methods[true_method], 
                             " estimate calculated with total sample size\nGenetic Architecture: ", name_GenArch))+
      theme_few(base_size = base_size_theta_plot) -> EGGER_GSD_simplot_thetas
    ## 7 ROBUST
    ROBUST_BMI_GSD_geom_point_dat <- est_sim %>% 
      select(!!!(colname_ROBUST_theta_sim_BMI_GSD)) %>% 
      pivot_longer(cols = everything(), names_to = "theta_sim") %>% 
    ...
    ggplot(ROBUST_GSD_performance_metrics, aes(x=N, y=ROBUST_GSD_mean))+ 
      ...
      labs(title = "Simulated Thetas with Empirical Standard Error", 
           subtitle = paste0("ROBUST estimate: BMI -> GSD\nSelected Reference Method: ", 
                             selected_true_methods[true_method], 
                             " estimate calculated with total sample size\nGenetic Architecture: ", name_GenArch))+
      theme_few(base_size = base_size_theta_plot) -> ROBUST_GSD_simplot_thetas
    ## 8 PRESSO
    PRESSO_BMI_GSD_geom_point_dat <- est_sim %>% 
      select(!!!(colname_PRESSO_theta_sim_BMI_GSD)) %>% 
      pivot_longer(cols = everything(), names_to = "theta_sim") %>% 
    ...
    ggplot(PRESSO_GSD_performance_metrics, aes(x=N, y=PRESSO_GSD_mean))+ 
      ...
      labs(title = "Simulated Thetas with Empirical Standard Error", 
           subtitle = paste0("PRESSO estimate: BMI -> GSD\nSelected Reference Method: ", 
                             selected_true_methods[true_method], 
                             " estimate calculated with total sample size\nGenetic Architecture: ", name_GenArch))+
      theme_few(base_size = base_size_theta_plot) -> PRESSO_GSD_simplot_thetas
    ## 9 RAPS
    RAPS_BMI_GSD_geom_point_dat <- est_sim %>% 
      select(!!!(colname_RAPS_theta_sim_BMI_GSD)) %>% 
      pivot_longer(cols = everything(), names_to = "theta_sim") %>% 
    ...
    ggplot(RAPS_GSD_performance_metrics, aes(x=N, y=RAPS_GSD_mean))+ 
      ...
      labs(title = "Simulated Thetas with Empirical Standard Error", 
           subtitle = paste0("RAPS estimate: BMI -> GSD\nSelected Reference Method: ", 
                             selected_true_methods[true_method], 
                             " estimate calculated with total sample size\nGenetic Architecture: ", name_GenArch))+
      theme_few(base_size = base_size_theta_plot) -> RAPS_GSD_simplot_thetas
    ## 10 LASSO
    LASSO_BMI_GSD_geom_point_dat <- est_sim %>% 
      select(!!!(colname_LASSO_theta_sim_BMI_GSD)) %>% 
      pivot_longer(cols = everything(), names_to = "theta_sim") %>% 
    ...
    ggplot(LASSO_GSD_performance_metrics, aes(x=N, y=LASSO_GSD_mean))+ 
      ...
      labs(title = "Simulated Thetas with Empirical Standard Error", 
           subtitle = paste0("LASSO estimate: BMI -> GSD\nSelected Reference Method: ", 
                             selected_true_methods[true_method], 
                             " estimate calculated with total sample size\nGenetic Architecture: ", name_GenArch))+
      theme_few(base_size = base_size_theta_plot) -> LASSO_GSD_simplot_thetas
    ...
    if (SAVE.files ==TRUE ){
      filename_GSD_simplot_thetas_arrange <- paste0("./output/plots/theta_sims/", Sys.Date(), "_GSD_simplot_thetas_REF_METHOD_", selected_true_methods[true_method],
    ...
    if (SAVE.files ==TRUE ){
      filename_simplot_bias_mse <- paste0("./output/plots/bias-mse/", Sys.Date(), "_GSD_bias_mse_REF_METHOD_",
                                          selected_true_methods[true_method], 
                                          "_GenArch_", name_GenArch, "_NSIM_", nsim,".svg")
      ggsave(filename = filename_simplot_bias_mse, plot = bias_mse_plot_GSD, 
             width = 7, height = 24, 
...
est.wald <- est.bmi.gsd$estimate[est.bmi.gsd$mr_methods == "FTO_Wald"]
est.mrmix <- est.bmi.gsd$estimate[est.bmi.gsd$mr_methods == "MRMix"]
...
df.temp.long[[1]] %>% 
  ...
  group_by(N) %>% 
  ## add columns for colouring closest to FTO
  mutate(closest_to_FTO_boolean_all = abs(Mean_ALL_INSTR - est.wald) == min(abs(Mean_ALL_INSTR - est.wald)),
         closest_to_FTO_boolean_strongcommon = abs(Mean_STRONG_COMMON - est.wald) == min(abs(Mean_STRONG_COMMON - est.wald)),
         closest_to_FTO_boolean_strongrare = abs(Mean_STRONG_RARE - est.wald) == min(abs(Mean_STRONG_RARE - est.wald)),
         closest_to_FTO_boolean_weakcommon = abs(Mean_WEAK_COMMON - est.wald) == min(abs(Mean_WEAK_COMMON - est.wald)),
         closest_to_FTO_boolean_weakrare = abs(Mean_WEAK_RARE - est.wald) == min(abs(Mean_WEAK_RARE - est.wald))
         ) %>% 
  ## add columns for colouring closest to MRMix
  mutate(closest_to_MRMIX_boolean_all = abs(Mean_ALL_INSTR - est.mrmix) == min(abs(Mean_ALL_INSTR - est.mrmix)),
         closest_to_MRMIX_boolean_strongcommon = abs(Mean_STRONG_COMMON - est.mrmix) == min(abs(Mean_STRONG_COMMON - est.mrmix)),
         closest_to_MRMIX_boolean_strongrare = abs(Mean_STRONG_RARE - est.mrmix) == min(abs(Mean_STRONG_RARE - est.mrmix)),
         closest_to_MRMIX_boolean_weakcommon = abs(Mean_WEAK_COMMON - est.mrmix) == min(abs(Mean_WEAK_COMMON - est.mrmix)),
         closest_to_MRMIX_boolean_weakrare = abs(Mean_WEAK_RARE - est.mrmix) == min(abs(Mean_WEAK_RARE - est.mrmix))
  ) %>% 
  gt(groupname_col="N") %>% 
  tab_header(
  ...
  ## colour closest values ----------------------------------------------
  # FTO
  tab_style(
    style = cell_text(color = "blue", weight = "bold"),
    locations = cells_body(
      columns = contains("Mean_ALL_INSTR"),
      rows = closest_to_FTO_boolean_all
    )
  ) %>% 
  tab_style(
    style = cell_text(color = "blue", weight = "bold"),
    locations = cells_body(
      columns = contains("Mean_STRONG_COMMON"),
      rows = closest_to_FTO_boolean_strongcommon
    )
  ) %>% 
  tab_style(
    style = cell_text(color = "blue", weight = "bold"),
    locations = cells_body(
      columns = contains("Mean_STRONG_RARE"),
      rows = closest_to_FTO_boolean_strongrare
    )
  ) %>% 
  tab_style(
    style = cell_text(color = "blue", weight = "bold"),
    locations = cells_body(
      columns = contains("Mean_WEAK_COMMON"),
      rows = closest_to_FTO_boolean_weakcommon
    )
  ) %>% 
  tab_style(
    style = cell_text(color = "blue", weight = "bold"),
    locations = cells_body(
      columns = contains("Mean_WEAK_RARE"),
      rows = closest_to_FTO_boolean_weakrare
    )
  ) %>% 
  # MRMix -----
  tab_style(
    ...
    locations = cells_body(
      columns = contains("Mean_ALL_INSTR"),
      rows = closest_to_MRMIX_boolean_all
    )
  ) %>% 
  tab_style(
    style = cell_text(color = "black", weight = "bold"),
    locations = cells_body(
      columns = contains("Mean_STRONG_COMMON"),
      rows = closest_to_MRMIX_boolean_strongcommon
    )
  ) %>% 
  tab_style(
    style = cell_text(color = "black", weight = "bold"),
    locations = cells_body(
      columns = contains("Mean_STRONG_RARE"),
      rows = closest_to_MRMIX_boolean_strongrare
    )
  ) %>% 
  tab_style(
    style = cell_text(color = "black", weight = "bold"),
    locations = cells_body(
      columns = contains("Mean_WEAK_COMMON"),
      rows = closest_to_MRMIX_boolean_weakcommon
    )
  ) %>% 
  tab_style(
    style = cell_text(color = "black", weight = "bold"),
    locations = cells_body(
      columns = contains("Mean_WEAK_RARE"),
      rows = closest_to_MRMIX_boolean_weakrare
    )
  ) %>% 
  #--------
  cols_merge(
  ...
  cols_hide(columns = c(closest_to_FTO_boolean_all, closest_to_FTO_boolean_strongcommon, closest_to_FTO_boolean_strongrare, closest_to_FTO_boolean_weakcommon, closest_to_FTO_boolean_weakrare,
                        closest_to_MRMIX_boolean_all, closest_to_MRMIX_boolean_strongcommon, closest_to_MRMIX_boolean_strongrare, closest_to_MRMIX_boolean_weakcommon, closest_to_MRMIX_boolean_weakrare)) %>% 
  tab_source_note(
  ...
  opt_stylize(style=1, color="gray") -> gt.tables.mean.se[[1]]

print(gt.tables.mean.se[[1]])

if (SAVE.files ==TRUE ){
  filename.gt.mean.se <- paste0("./output/gt-tables/sim-results/mean-empse/", Sys.Date(), 
                                        "_mean_emp_se_performance_metrics_NSIM_", nsim)
...
## loop to create tables with reference change
for (ref.method in seq_along(selected_true_methods)) {

  # TABLE 02: BIAS, MSE, COVERAGE, TYPE1ERROR ---------------------------------
  if(selected_true_methods[ref.method] == "FTO_Wald") {
    highlightcol <- "blue"
  } else if (selected_true_methods[ref.method] == "MRMix") {
    highlightcol <- "black"
  } else {highlightcol <- ggthemes_data$few$colors$Dark[9,2][[1]]}
  ...
  df.temp.long[[ref.method]] %>%
    ...
    mutate(N = paste0("Sample Size = ", format(N, scientific=FALSE))) %>% 
    group_by(N) %>% 
    ## add columns for colouring bias
    mutate(minbias_all = abs(bias_ALL_INSTR) ==  min(abs(bias_ALL_INSTR)),
           minbias_strongcommon = abs(bias_STRONG_COMMON) == min(abs(bias_STRONG_COMMON)),
           minbias_strongrare = abs(bias_STRONG_RARE) == min(abs(bias_STRONG_RARE)),
           minbias_weakcommon = abs(bias_WEAK_COMMON) == min(abs(bias_WEAK_COMMON)),
           minbias_weakrare = abs(bias_WEAK_RARE) == min(abs(bias_WEAK_RARE)),
           # cols for mse
           minmse_all = abs(mse_ALL_INSTR) == min(abs(mse_ALL_INSTR)),
           minmse_strongcommon = abs(mse_STRONG_COMMON) == min(abs(mse_STRONG_COMMON)),
           minmse_strongrare = abs(mse_STRONG_RARE) == min(abs(mse_STRONG_RARE)),
           minmse_weakcommon = abs(mse_WEAK_COMMON) == min(abs(mse_WEAK_COMMON)),
           minmse_weakrare = abs(mse_WEAK_RARE) == min(abs(mse_WEAK_RARE)),
           # cols for coverage
           mincoverage_all = coverage_ALL_INSTR == max(coverage_ALL_INSTR),
           mincoverage_strongcommon = coverage_STRONG_COMMON == max(coverage_STRONG_COMMON),
           mincoverage_strongrare = coverage_STRONG_RARE == max(coverage_STRONG_RARE),
           mincoverage_weakcommon = coverage_WEAK_COMMON == max(coverage_WEAK_COMMON),
           mincoverage_weakrare = coverage_WEAK_RARE == max(coverage_WEAK_RARE),
           # cols for type1error
           mintype1error_all = type1error_ALL_INSTR == max(type1error_ALL_INSTR),
           mintype1error_strongcommon = type1error_STRONG_COMMON == max(type1error_STRONG_COMMON),
           mintype1error_strongrare = type1error_STRONG_RARE == max(type1error_STRONG_RARE),
           mintype1error_weakcommon = type1error_WEAK_COMMON == max(type1error_WEAK_COMMON),
           mintype1error_weakrare = type1error_WEAK_RARE == max(type1error_WEAK_RARE),
    ) -> gtperf.dat
    # make {gt} table
  gtperf.dat %>% 
    gt(groupname_col="N") %>% 
    ...
    # bias
    tab_style(
      style = cell_text(color = highlightcol, weight = "bold"),
      locations = cells_body(
        columns = contains("bias_ALL_INSTR"),
        rows = minbias_all
      )
    ) %>% 
      tab_style(
        style = cell_text(color = highlightcol, weight = "bold"),
        locations = cells_body(
          columns = contains("bias_STRONG_COMMON"),
          rows = minbias_strongcommon
        )
      ) %>% 
      tab_style(
        style = cell_text(color = highlightcol, weight = "bold"),
        locations = cells_body(
          columns = contains("bias_STRONG_RARE"),
          rows = minbias_strongrare
        )
      ) %>% 
      tab_style(
        style = cell_text(color = highlightcol, weight = "bold"),
        locations = cells_body(
          columns = contains("bias_WEAK_COMMON"),
          rows = minbias_weakcommon
        )
      ) %>% 
      tab_style(
        style = cell_text(color = highlightcol, weight = "bold"),
        locations = cells_body(
          columns = contains("bias_WEAK_RARE"),
          rows = minbias_weakrare
        )
      ) %>% 
    # mse
    tab_style(
      style = cell_text(color = highlightcol, weight = "bold"),
      locations = cells_body(
        columns = contains("mse_ALL_INSTR"),
        rows = minmse_all
      )
    ) %>% 
    tab_style(
      style = cell_text(color = highlightcol, weight = "bold"),
      locations = cells_body(
        columns = contains("mse_STRONG_COMMON"),
        rows = minmse_strongcommon
      )
    ) %>% 
    tab_style(
      style = cell_text(color = highlightcol, weight = "bold"),
      locations = cells_body(
        columns = contains("mse_STRONG_RARE"),
        rows = minmse_strongrare
      )
    ) %>% 
    tab_style(
      style = cell_text(color = highlightcol, weight = "bold"),
      locations = cells_body(
        columns = contains("mse_WEAK_COMMON"),
        rows = minmse_weakcommon
      )
    ) %>% 
    tab_style(
      style = cell_text(color = highlightcol, weight = "bold"),
      locations = cells_body(
        columns = contains("mse_WEAK_RARE"),
        rows = minmse_weakrare
      )
    ) %>% 
    # coverage
    tab_style(
      style = cell_text(color = highlightcol, weight = "bold"),
      locations = cells_body(
        columns = contains("coverage_ALL_INSTR"),
        rows = mincoverage_all
      )
    ) %>% 
    tab_style(
      style = cell_text(color = highlightcol, weight = "bold"),
      locations = cells_body(
        columns = contains("coverage_STRONG_COMMON"),
        rows = mincoverage_strongcommon
      )
    ) %>% 
    tab_style(
      style = cell_text(color = highlightcol, weight = "bold"),
      locations = cells_body(
        columns = contains("coverage_STRONG_RARE"),
        rows = mincoverage_strongrare
      )
    ) %>% 
    tab_style(
      style = cell_text(color = highlightcol, weight = "bold"),
      locations = cells_body(
        columns = contains("coverage_WEAK_COMMON"),
        rows = mincoverage_weakcommon
      )
    ) %>% 
    tab_style(
      style = cell_text(color = highlightcol, weight = "bold"),
      locations = cells_body(
        columns = contains("coverage_WEAK_RARE"),
        rows = mincoverage_weakrare
      )
    ) %>% 
    # type1error
    tab_style(
      style = cell_text(color = highlightcol, weight = "bold"),
      locations = cells_body(
        columns = contains("type1error_ALL_INSTR"),
        rows = mintype1error_all
      )
    ) %>% 
    tab_style(
      style = cell_text(color = highlightcol, weight = "bold"),
      locations = cells_body(
        columns = contains("type1error_STRONG_COMMON"),
        rows = mintype1error_strongcommon
      )
    ) %>% 
    tab_style(
      style = cell_text(color = highlightcol, weight = "bold"),
      locations = cells_body(
        columns = contains("type1error_STRONG_RARE"),
        rows = mintype1error_strongrare
      )
    ) %>% 
    tab_style(
      style = cell_text(color = highlightcol, weight = "bold"),
      locations = cells_body(
        columns = contains("type1error_WEAK_COMMON"),
        rows = mintype1error_weakcommon
      )
    ) %>% 
    tab_style(
      style = cell_text(color = highlightcol, weight = "bold"),
      locations = cells_body(
        columns = contains("type1error_WEAK_RARE"),
        rows = mintype1error_weakrare
      )
    ) %>% 
    # --------------------------------------------------------------------------
    cols_hide(columns = colnames(gtperf.dat)[23:42]) %>% 
  ...
  print(gt.tables.performance2[[ref.method]])
...
```{r summary-plots-all-1, figures-side, fig.show='hold', out.width="100%", warning=FALSE, message=FALSE}
...
for(ref.method in seq_along(df_metrics_all_scenarios)) {
  ...
  if (names(df_metrics_all_scenarios)[ref.method] == "ref_MRMix") {
    color.bar <- "black"
  } else if (names(df_metrics_all_scenarios)[ref.method] == "ref_FTO_Wald"){
    color.bar <- "blue"
  } else {
    color.bar <- ggthemes_data$few$colors$Dark[9,2][[1]]
  }
  ...
  for(scenario in seq_along(scenarios)) {
    ...
    df.temp.long[[ref.method]] %>%
      ...
      drop_na(Mean) %>%  
      group_by(N) %>% 
      mutate(smallest_mse = mse == min(mse)) %>% # add a new column to identify the rows with the smallest mse value within each group
      ungroup() %>% 
      # PLOT
      ggplot(aes(x=Method, y=mse)) +
      geom_bar(
        aes(fill = smallest_mse), # add fill aesthetics to highlight the bars with the smallest mse value within each group
        stat="identity", position=position_dodge2(reverse=TRUE),
        #color=ggthemes_data$few$colors$Dark[2,2][[1]],
      ) +   # reverse dodge
      ...
      scale_fill_manual(values = c("gray80", color.bar), guide = "none") -> dodged.barplot.mse.scenario[[scenario]]
  ...
  dodged.barplot.mse.references[ref.method] <- dodged.barplot.mse.scenario[1]   # save scenario 1 "ALL INSTR"
  
  # scenarios WITHOUT all instruments in arranged plot
  
  # make smaller plots
  base_theme <- theme_few(base_size = 8)
  for (smallplot in seq_along(dodged.barplot.mse.scenario)) {
    dodged.barplot.mse.scenario[[smallplot]] <- dodged.barplot.mse.scenario[[smallplot]] +
      base_theme
  }
  
  arrangedplot.mse.scenarios <- 
    ggarrange(plotlist = dodged.barplot.mse.scenario[-1], labels = c("Scenario 2", "Scenario 3", "Scenario 4", "Scenario 5"))
  ...
  plot(dodged.barplot.mse[[ref.method]])
  ...
  for(scenario in seq_along(scenarios)) {
    ...
    df.temp.long[[ref.method]] %>%
      ...
      drop_na(Mean) %>%  
      group_by(N) %>% 
      mutate(smallest_bias = abs(bias) == min(abs(bias))) %>% # add a new column to identify the rows with the smallest bias value
      ungroup() %>% 
      # PLOT
      ggplot(aes(x=Method, y=bias)) +
      geom_bar(
        aes(fill = smallest_bias), # add fill aesthetics to highlight the bars with the smallest mse value within each group
        stat="identity", position=position_dodge2(reverse=TRUE),
        #color=ggthemes_data$few$colors$Dark[2,2][[1]],
      ) +   # reverse dodge
      ...
      coord_flip()+
      ggthemes::theme_few()+  # remove legend
      scale_fill_manual(values = c("gray80", color.bar), guide = "none") -> dodged.barplot.bias.scenario[[scenario]]
  ...
  # make smaller plots
  base_theme <- theme_few(base_size = 8)
  for (smallplot in seq_along(dodged.barplot.bias.scenario)) {
    dodged.barplot.bias.scenario[[smallplot]] <- dodged.barplot.bias.scenario[[smallplot]] +
      base_theme
  }
  
  arrangedplot.bias.scenarios <- 
    ggarrange(plotlist = dodged.barplot.bias.scenario[-1], labels = c("Scenario 2", "Scenario 3", "Scenario 4", "Scenario 5"))
  ...
  plot(dodged.barplot.bias[[ref.method]])
  ...
  plot(dodged.barplot.coverage[[ref.method]])
  ...
  plot(dodged.barplot.power[[ref.method]])
...
plot(arrangedplot.mse.references)

if (SAVE.files ==TRUE ){
    ## arranged plots are toooo small 
    filename.dodged.barplot.mse <- paste0("./output/plots/summary-plots/arranged_references/", 
                                          Sys.Date(), "_mse_GSD_dodged_barplot_REF_METHOD_",
                                          selected_true_methods[ref.method], "_NSIM_", nsim)
...
plot(arrangedplot.bias.references)
...
# Figure 10/11 MSE and BIAS in Scenario 1 with ref estimate FTO Wald
names(dodged.barplot.mse.references) <- names(df_metrics_all_scenarios)
names(dodged.barplot.bias.references) <- names(df_metrics_all_scenarios)
arranged.msebias.scenario1.fto <- ggarrange(dodged.barplot.mse.references$ref_FTO_Wald, dodged.barplot.bias.references$ref_FTO_Wald, ncol=2, labels = c("A", "B"))
arranged.msebias.scenario1.mrmix <- ggarrange(dodged.barplot.mse.references$ref_MRMix, dodged.barplot.bias.references$ref_MRMix, ncol=2, labels = c("A", "B"))

if (SAVE.files ==TRUE ){
    ## arranged plots are toooo small 
    filename.arranged.msebias.scenario1.fto <- paste0("./output/plots/summary-plots/figure10/", Sys.Date(), "_bias_mse_barplot_REF_METHOD_FTO_WALD_NSIM_", nsim)
    filename.arranged.msebias.scenario1.mrmix <- paste0("./output/plots/summary-plots/figure10/", Sys.Date(), "_bias_mse_barplot_REF_METHOD_MRMIX_NSIM_", nsim)
    
    ggsave(filename = paste0(filename.arranged.msebias.scenario1.fto, ".svg"), plot =  arranged.msebias.scenario1.fto, 
             width = 8, height = 7, 
             units = "in"  # default
             )
    ggsave(filename = paste0(filename.arranged.msebias.scenario1.mrmix, ".svg"), plot =  arranged.msebias.scenario1.mrmix, 
             width = 8, height = 7, 
             units = "in"  # default
             )
}
...
# map(names(sessionInfo()$otherPkgs), ~citation(.x))
```
