...
# data FOR simulation in simulation script
...
funs <- c(bias = bias, mse = mse)
args <- list(theta = est$estimate[est$mr_methods == "conmix"]) # trim argument non-matching and ignored for median
...
```{r plot-performance-metrics-bias-mse}
#BIAS
...
```{r plot-performance-metrics-boxplot}
...
ggplot(boxplot_dat, aes(x=forcats::fct_rev( as.factor(theta_sim_numeric)), y=value)) +
  ...
  coord_flip()+
  theme_few() -> simplot_betas
simplot_betas


filename_simplot_betas <- paste0("../output/plots/", Sys.Date(), "_figure_simplot_betas.png")
ggsave(filename = filename_simplot_betas, plot = simplot_betas, 
       width = 10, height = 6, 
       units = "in"  # default
       )
```
