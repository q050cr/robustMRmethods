...
plotfigure01.withGBC <- function(data, effect_size="all", maf="all", n){
  ## data: my_data_harm
  ...
  filenamefig01_quadrant <- paste0("./output/plots/pve/plot-fn-pve/", Sys.Date(), "_figure01_", 
...
plotfigure01 <- function(data, effect_size="all", maf="all", n){
  ...
  plot_data1 <- df_cum_expl_var %>% 
    pivot_longer(cols = c(cum_expl_varX, cum_expl_varU)) %>% 
    mutate(name = factor(name, labels = c("% var(Y)", "% var(X)"))) 
  ...
  if( effect_size=="all" & maf=="all"  ){
    ggplot(plot_data1, aes(x=n, y=value, col=name))+
      ...
      #                                         ))
      #
      scale_color_manual(name = "", values = c("% var(X)" = ggthemes_data$few$colors$Dark[3,2][[1]], 
                                               "% var(Y)" = ggthemes_data$few$colors$Dark[4,2][[1]], 
                                               "Number of IVs" = "black"),
                         # order
                         breaks = c( "Number of IVs", "% var(X)", "% var(Y)")
      ) -> figure01.quadrant
